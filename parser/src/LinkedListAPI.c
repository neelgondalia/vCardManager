 ...
 * @author  CIS*2750 S18 (based on the ListADT from CIS*2520, S17) / Neel Gondalia (0940429)
 * @date September 2018
...
void clearList(List *list)
{
	...
	Node *headOfList = list->head;
	while (headOfList != NULL)
	{
		Node *toBeCleared = headOfList;
		headOfList = headOfList->next;

		list->deleteData(toBeCleared->data);
		free(toBeCleared);
	}
